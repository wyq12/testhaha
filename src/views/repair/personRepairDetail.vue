<template>
  <div>
    <div class="repairBox">
      <div class="comBox">
        <div class="comBackground">报修详情(个人)</div>
        <div class="RepairDEtailContent">
          <div class="header dealStateBox">
            <div class="headerBtn">
              ...
              <div class="containBtn fontSize">
                <router-link
                  :to="{path:'/progressDetail',query:{repairType:'person',orderNo:orderNo}}"
                >进度明细</router-link>
              </div>
            </div>
            <div class="dealTime">
          ...
          <div class="repairInfo">
            ...
            <div>
              <span>预约时间：</span>
              <span v-cloak>{{hopeDealTime}}</span>
            </div>
            <div>
              <span>报修类型：</span>
              <span v-cloak>{{repairType}}</span>
            </div>
            <div>
              <span>报修地址：</span>
              <span v-cloak>{{repairAddr}}</span>
            </div>
            <div>
              <span>收费金额：</span>
              <span v-if="state<2">--</span>
              <span v-if="state==4">--</span>
              <template v-if="state>1">
                <span v-if="state<4" v-cloak>{{amount}}</span>
                <template v-if="state<4">
                  <span v-if="amount=='0'">(收费金额为0,无法开发票)</span>
                </template>
              </template>
              <template v-if="amount!='0'">
                <div
                  class="containBtn"
                  v-if="state==2"
                  style="float:right;margin-bottom:0;height:1.2rem;line-height:1.2rem;"
                >
                  <router-link
                    :to="{path:'/personRepairPay',query:{orderNo:orderNo,resCode:resCode,dictId:dictId}}"
                  >付款</router-link>
                </div>
              </template>
            </div>
            ...
            <div>
              <span>是否需要开发票：</span>
              <span v-if="isNeedInvoice=='1'">是</span>
              <span v-if="isNeedInvoice=='0'">否</span>
            </div>
            <!-- 处理中 -->
            <template v-if="state>0">
              <div v-if="state<4" class="flexs">
                <span>处理人员：</span>
            ...
            <template v-if="state==3">
              ...
              <div>
                <span>实付金额：</span>
                <span v-cloak>{{amount}}</span>
              </div>
            </template>
            <div>
              <span>内容：</span>
              <span v-cloak>{{repairContent}}</span>
            </div>
            <div class="addPhoto">
              <div v-if="repairPhoto.length>0">图片：</div>
              <div class="photoContent">
                <div class="photo" v-for="(item,index) in repairPhoto" :key="index">
                  <img :src="item.src" @click="funcReadImgInfo(index,item.src)" alt />
                </div>
              </div>
            </div>
            <div v-if="state>1">
            ...
            <template v-if="state>1">
              <div style="margin-bottom:15px;" v-if="state<4">
                ...
                <div class="addPhoto" v-if="state==2 && amount=='0'">
            ...
            <div style="text-align:center;">
              <mt-button class="cancel" size="large" v-if="state==0" @click="cancel">取消订单</mt-button>
              <template v-if="amount=='0'">
                <mt-button
                  type="default"
                  v-if="state==2"
                  @click="submitEvaluates"
                  class="containBtn"
                >提交</mt-button>
                <mt-button type="default" v-if="state==2" class="containBtn">
    ...
    <!-- <div v-if="zzShow" class="img-view" @click= "viewImg" v-cloak >
    　　　　 <div class="img-layer" @touchmove.prevent></div>
    　　　　 <div class="imgs">
                  <mt-swipe :auto="0" :default-index = 'defaultSrc'>
                    <mt-swipe-item  v-for="(item,index) in repairPhoto" :key = 'index'><img :src="item.src" alt=""></mt-swipe-item>
                  </mt-swipe>
    　　　　 </div>
    </div>-->
    <div
...
export default {
  ...
  methods: {
    async infodetail() {
      ...
      await infoDetailPort({
        ...
        if (rtnData.info == 'success') {
          ...
          console.log(rtnInfo.perOrderNo)
          ...
          that.repairType = rtnTypeJson.dictNameJson;
          ...
          that.perOrderNo = rtnInfo.perOrderNo;
          that.isNeedInvoice = rtnInfo.isNeedInvoice;
          that.amount = rtnInfo.amount;
          that.hopeDealTime = rtnInfo.hopeDealTime == '' ? '--' : rtnInfo.hopeDealTime;
          that.payDate = Util.fmDay(rtnInfo.payDate);
        ...
        .then(res => {
          ...
          if (rtnData.info == 'success') {
            for (let i = 0; i < imgSrc.imgList.length; i++) {
              var imgArr = {
                src: imgSrc.imagePath + imgSrc.imgList[i]
              }
              that.repairPhoto.push(imgArr)
            }
    ...
    zhuHuRemind: function (that) {
      ...
      zhuHuRemindPort(params).then(res => {
        let rtnData = res.data;
        if (rtnData.info == "success") {
          Toast("提醒成功");
